...
# Extracción señal de cada estrella
  ...
  stdev = std
  phot_table[name_mag + '_mag_err'] = 1.0857 * np.sqrt(np.abs(final_sum/epadu + area_aper*stdev**2))/final_sum
...
#----# Se guardan las tablas como archivos .csv
  ...
  for j in filtro_final[foc]:
    ...
    counter += 1
  final_obs_table.write(f'./WISE_output/Table_{foc}.csv', overwrite=True)    
