...
#Lambda = np.array([3.6e-4, 4.5e-4, 5.8e-4, 8.0e-4, 3.35e-4, 4.6e-4, 11.6e-4, 21.1e-4, 23.68e-4])
Fzero = np.array([6.48148e-8, 2.66222e-8, 1.02556e-8, 3.00609e-9, 8.17870e-8, 6.51510e-10, 5.09010e-11, 3.73438e-11])

#print(mag["abs_2_mag_0_W"])
...
fluxes['flux_abs_1_mag_0'] = Fzero[7] * 10**((-0.4)*mag['abs_1_mag_0'].to_numpy())
...
for index, row in flux_df.iterrows():
    ...
    print(int(row['OBJECT_ID']), flux_lambda_sorted, "\t")

    # Graficar
    plt.scatter(Lambda_sorted, np.log(flux_lambda_sorted), color='red')
    plt.plot(Lambda_sorted, np.log(flux_lambda_sorted), linestyle='-', linewidth=2)
